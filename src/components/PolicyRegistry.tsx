    ...
    <div className="mx-auto max-w-6xl space-y-8 animate-fade-in">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1.5 max-w-3xl text-sm text-slate-600 dark:text-slate-400">See the safeguards the gateway applies to agent actions. Policy changes are available to authorized operators.</p>
        </div>
      ...
      <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs text-amber-900 dark:border-amber-900/50 dark:bg-amber-950/20 dark:text-amber-200">
        <div className="flex items-start gap-2"><ShieldAlert className="mt-0.5 h-4 w-4 shrink-0" /><span>Some safeguards are mandatory. Optional safeguards can be enabled or disabled by an authorized operator.</span></div>
      </div>
        ...
        {loading && policies.length === 0 ? [1, 2, 3, 4].map((item) => <div key={item} className="h-48 animate-pulse rounded-2xl bg-slate-100 dark:bg-slate-900" />) : policies.map((policy) => {
          ...
          return <article key={policy.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900/70">
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="flex items-center gap-2"><span className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase ${policy.severity === 'CRITICAL' ? 'bg-rose-100 text-rose-800 dark:bg-rose-500/20 dark:text-rose-300' : 'bg-amber-100 text-amber-800 dark:bg-amber-500/20 dark:text-amber-300'}`}>{policy.severity}</span></div>
            ...
            <div className="mt-4 space-y-2 text-[11px]">
              <details className="rounded-lg bg-slate-50 p-3 dark:bg-slate-950/60">
                <summary className="cursor-pointer text-slate-600 dark:text-slate-400">View matching rule</summary>
                <div className="mt-2 break-anywhere font-mono text-slate-700 dark:text-slate-300">{policy.rule_expression}</div>
              </details>
              ...
              <div className="flex items-center gap-1.5 font-semibold text-slate-500 dark:text-slate-400">{enabled ? <CheckCircle2 className="h-3.5 w-3.5 text-emerald-500" /> : <ToggleLeft className="h-3.5 w-3.5" />}{immutable ? 'Mandatory' : enabled ? 'Enabled' : 'Disabled'}</div>
            </div>
          </article>;
        })}
      </div>
